...
class Matrix:
    ...
    def matrixm0(self, B, accuracy):
        if (self.len[1] == B.len[0]):
            R = Matrix([], "Result")
            R.makezeromatrix(self.len[0])
            for i in range(0, self.len[0]):
                for j in range(0, B.len[1]):
                    R.matrix[i][j] += round(self.matrix[i][j] * B.matrix[j][i], accuracy)
            return R
        else:
...
class Vector:
    ...
    def rawsubtract(self, B, accuracy):
        if (self.len == B.len):
            R = self.copy()
            R.rename("Result")
