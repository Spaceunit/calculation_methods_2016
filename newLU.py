...
class LU:
    ...
    def setlu(self):
        ...
        for raw in range(0,self.um.len[0]):
    ...
    def numberconversion(self,raw):
        ...
        num = self.um.getel(raw, raw)
        print("R",raw + 1,"/",num,";")
        for i in range (raw,self.um.len[0]):
            print("Step #",i + 1)
            self.um.chel(raw,i,round(self.um.getel(raw,i) / num, self.accuracy))
            self.um.showmatrix()
    ...
    def rawsubtract(self,raw):
        ...
        print("Row subtract...")
        print("Step#", raw + 1, ";")
        for k in range(raw + 1,self.um.len[0]):
            print("R", k + 1, "- R", k, "*", "R(", k + 1, ",", raw + 1, ")")
            if math.copysign(1, self.um.getel(raw, raw)) == math.copysign(1, self.um.getel(k, raw)):
                self.lm.chel(k, raw, round(self.um.getel(k, raw) / self.um.getel(raw, raw), self.accuracy))
                pass
            else:
                self.lm.chel(k, raw, round(-1 * self.um.getel(k, raw) / self.um.getel(raw, raw), self.accuracy))
                pass
            #self.lm.chel(k,raw,self.um.getel(k,raw) / self.um.getel(raw,raw))
            #self.lm.chel(k,raw,self.um.getel(k,raw))
            self.um.chel(k,raw,0)
            for j in range(raw + 1,self.um.len[0]):
                print("Step #", j + 1)
                self.um.chel(k, j, round(self.um.getel(k,j) - self.um.getel(raw,j) * self.lm.getel(k,raw), self.accuracy))
                self.um.showmatrix()
                self.lm.showmatrix()
                pass
